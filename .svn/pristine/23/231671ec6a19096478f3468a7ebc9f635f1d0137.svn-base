...
public class TimeoutEmailBean implements java.io.Serializable {
	...
	public void createTimeoutJob() {
		try {
			...
			showMessage(new FacesMessage(FacesMessage.SEVERITY_ERROR, "创建定时器失败", ""));
	...
	public void editTimeoutJob() {
		try {
			...
			showMessage(new FacesMessage(FacesMessage.SEVERITY_ERROR, "更新定时器失败", ""));
	...
	public boolean validateJobInfoVo(JobInfoVo jobInfoVo) {
		...
		if (!ValidateUtil.validateStartTimeGTEndTime(context, new Date(), jobInfoVo.getEndDate(), "结束时间：", "必须晚于当前时间。")) {
			validate = false;
		}
		return validate;
	}
