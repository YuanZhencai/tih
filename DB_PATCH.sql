...
alter table COMPANY_LAND_DETAILS alter column LAND_USAGE SET DATA TYPE VARCHAR(300);
alter table COMPANY_LAND_DETAILS alter column LAND_KIND SET DATA TYPE VARCHAR(300);
alter table COMPANY_LAND_DETAILS alter column DEV_DEGREE SET DATA TYPE VARCHAR(300);
alter table COMPANY_LAND_DETAILS alter column CERTIFICATE_RIGHT_BY SET DATA TYPE VARCHAR(300);
CALL sysproc.admin_cmd('reorg table COMPANY_LAND_DETAILS');


CREATE OR REPLACE VIEW VW_ORG_AND_COM (ID,OID,STEXT,PARENT,ZHRZZCJID) AS
WITH vw_porg(ID,OID,STEXT,PARENT,ZHRZZCJID) AS (
SELECT DISTINCT ID,ID AS OID ,STEXT,PARENT,ZHRZZCJID FROM O
UNION ALL
SELECT o.ID,r.OID ,o.STEXT,o.PARENT,o.ZHRZZCJID FROM O o, vw_porg r WHERE o.ID= r.PARENT)
SELECT DISTINCT ID,OID,STEXT,PARENT,ZHRZZCJID FROM vw_porg vw WHERE ZHRZZCJID LIKE '4%';
-- ADD POSITION_REMARK
