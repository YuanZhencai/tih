...
//		xsql.append(" left join vw_org_and_com c on p.orgeh = c.oid");
		xsql.append(" left join O o on o.bukrs = p.bukrs and o.zhrzzcjid = '40'");
		xsql.append(" where 1 = 1 ");

		xsql.append(" /~ and p.nachn like {username} ~/ ");
		xsql.append(" /~ and u.position_remark like {position} ~/ ");
		xsql.append(" /~ and (u.position_remark <> {other} or u.position_remark is null) ~/ ");
		xsql.append(" /~ and o.stext like {company} ~/ ");
		xsql.append(" /~ and u.defunct_ind = {defunctInd} ~/ ");
		if (!isCount) {
			xsql.append(" order by o.id");
		}
