...
public class ApplyQuestionBean implements Serializable {
	...
	private boolean showCacheButton = false;

	// ===========================================Yuan=========================================//
	public void saveCache() {
		List<WfStepmstr> steps = wfIns.getWfStepmstrs();
		if (steps == null || steps.size() == 0) {
			JSFUtils.addMessage(new FacesMessage(FacesMessage.SEVERITY_ERROR, "操作失败：", "请刷新页面重新操作"));
			return;
		}
		...
		questionVo.setFileDetails(processDlgFileList);
		questionVo.setRecipientsMap(recipientsMap);
		questionVo.setSupervisor(supervisor);
		questionVo.setShowSupervisor(showSupervisor);

		String key = loginService.getCurrentUserName() + "_" + wfIns.getId() + "_" + steps.get(steps.size() - 1).getId();

		Cache cache = new Cache();
		cache.setKey(key);
		cache.setValue(questionVo);

		cacheManagerService.set(key, cache);
		JSFUtils.addMessage(new FacesMessage(FacesMessage.SEVERITY_INFO, "操作成功", ""));
		JSFUtils.handleDialogByWidgetVar("applyQuestionDlgVar", "close");
	}

	private void getCache() {
		List<WfStepmstr> steps = wfIns.getWfStepmstrs();
		if (steps == null || steps.size() == 0) {
			logger.info("[wfIns error]");
			return;
		}
		String key = loginService.getCurrentUserName() + "_" + wfIns.getId() + "_" + steps.get(steps.size() - 1).getId();
		Cache cache = cacheManagerService.get(key);
		if (cache != null) {
			QuestionVo questionVo = (QuestionVo) cache.getValue();
			txtOpionion = questionVo.getTxtOpionion();
			processDlgParamMap = questionVo.getProcessDlgParamMap();
			recipientsSelectList = questionVo.getRecipientsSelectList();
			wfRemindVo = questionVo.getWfRemindVo();
			processDlgFileList = questionVo.getFileDetails();
			recipientsMap = questionVo.getRecipientsMap();
			supervisor = questionVo.getSupervisor();
			showSupervisor = questionVo.getShowSupervisor();
		}
	...
	public void showWorkflowProcessData() {
		...
		List<WfStepmstr> steps = wfIns.getWfStepmstrs();
		if (steps != null && steps.size() > 0) {
			if (loginService.getCurrentUserName().equals(steps.get(steps.size() - 1).getChargedBy())) {
	...
	private void workflowMethod(String workflowNodalPoint, String sendDirection) {
		...
		try {
			...
			if (steps != null && steps.size() > 0) {
				String key = loginService.getCurrentUserName() + "_" + wfIns.getId() + "_" + steps.get(steps.size() - 1).getId();
				cacheManagerService.remove(key);
			}
	...
	public boolean isShowCacheButton() {
		return showCacheButton;
	}

	public void setShowCacheButton(boolean showCacheButton) {
		this.showCacheButton = showCacheButton;
	}
