package com.wcs.tih.system.controller.util;
...
/**
 * <p>
 * Project: RegexValidator Description:为自定义校验器开发专用标签--自定义校验器类 Copyright (c) 2012 Wilmar Consultancy Services All Rights
 * Reserved.
 * @version 15082版本之后的主要修改为:throw new ValidatorException全部改为了FacesContext.getCurrentInstance().addMessage,避免验证红框!
 * @author <a href="mailto:zhaoqian@wcs-global.com">zhaoqian</a>
 */
@FacesValidator("customValidator")
public class CustomValidator implements Validator {

    private Logger logger = LoggerFactory.getLogger(CustomValidator.class);
    ...
    public void validate(FacesContext context, UIComponent component, Object toValidate) throws ValidatorException {
        ...
        if (required != null) {
            ...
            if (value.equals("")) {
                ...
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, requiredMessage == null ? errorMessage : requiredMessage, ""));
            }
            // 为空格的情况
        ...
        if (minLength != null) {
            ...
            try {
                if (value.getBytes("UTF-8").length < min) {
                    String errorMessage = this.disposePlaceholder(rb, "msg_minLength", minLengthMark);
                    throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, minLengthMessage == null ? errorMessage : minLengthMessage, ""));
                }
            } catch (UnsupportedEncodingException e) {
                logger.error(e.getMessage(), e);
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "不支持的字符编码集!请联系管理员", ""));
            }
        ...
        if (maxLength != null) {
            ...
            try {
                if (value.getBytes("UTF-8").length > max) {
                    ...
                    throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, maxLengthMessage == null ? errorMessage : maxLengthMessage, ""));
                }
            } catch (UnsupportedEncodingException e) {
                logger.error(e.getMessage(), e);
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "不支持的字符编码集!请联系管理员", ""));
        ...
        // 如果对应的regex,mark,message不匹配,输出错误信息.
            ...
            for (int i = 0; i < regexArray.length; i++) {
                ...
                try {
                    ...
                    if (!value.matches(regexArray[i])) {
                        ...
                        throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, regexMessageArray[i] == null ? errorMessage : regexMessageArray[i], ""));
                    } else {
                        continue;
                    }
                } catch (Exception e) {
                ...
                if (!(value).matches(backgroundRegex)) {
                    ...
                    try {
                        ...
                        String tempStr = regexrb.getString(regexArray[i]);
                    } catch (MissingResourceException e) {
                        logger.error(e.getMessage(), e);
