...
public class CompanyStockService {
	...
	public List<StockVo> findStockVosBy(Long companyId, Map<String, Object> filter) {
		StringBuilder jpql = new StringBuilder();
		jpql.append(" select c from CompanyStockStructure c");
		jpql.append(" where c.companymstr.id = ").append(companyId);
		if (filter != null) {
			Object statisticsDatetime = filter.get("statisticsDatetime");
			if (statisticsDatetime != null) {
				jpql.append(" and c.statisticsDatetime = '" + DateUtils.format((Date) statisticsDatetime, "yyyy-MM-dd 00:00:00") + "'");
			} else {
				if (filter.get("name") != null && !filter.get("name").equals("")) {
					jpql.append(" and c.shareholder like '%").append(filter.get("name")).append("%'");
				}
				if (filter.get("defunctInd") != null && !filter.get("defunctInd").equals("")) {
					jpql.append(" and c.defunctInd = '").append(filter.get("defunctInd")).append("'");
				}
				Object startdate = filter.get("startdate");
				if (startdate != null) {
					jpql.append(" and c.statisticsDatetime > '").append(DateUtils.format((Date) startdate, "yyyy-MM-dd 00:00:00")).append("'");
				}
				Object endDate = filter.get("endDate");
				if (endDate != null) {
					jpql.append(" and c.statisticsDatetime < '").append(DateUtils.format((Date) endDate, "yyyy-MM-dd 00:00:00")).append("'");
				}
			}
		}
		jpql.append(" order by c.statisticsDatetime");
		List<CompanyStockStructure> stocks = em.createQuery(jpql.toString()).getResultList();
		List<StockVo> stockVos = new ArrayList<StockVo>();
		for (CompanyStockStructure stock : stocks) {
			stockVos.add(getStockVoBy(stock));
		}
		return stockVos;

	}

	public StockVo getStockVoBy(CompanyStockStructure stock) {
		StockVo stockVo = new StockVo();
		stockVo.setCompanymstr(stock.getCompanymstr());
		stockVo.setCreatedBy(stock.getCreatedBy());
		stockVo.setCreatedDatetime(stock.getCreatedDatetime());
		stockVo.setCurrency(stock.getCurrency());
		stockVo.setDefunctInd(stock.getDefunctInd());
		stockVo.setRatio(stock.getRatio() + "");
		stockVo.setRegisteredCapital(stock.getRegisteredCapital() + "");
		stockVo.setShareholder(stock.getShareholder());
		stockVo.setStatisticsDatetime(stock.getStatisticsDatetime());
		stockVo.setStock(stock);
		stockVo.setType(stock.getType());
		stockVo.setUpdatedBy(stock.getUpdatedBy());
		stockVo.setUpdatedDatetime(stock.getUpdatedDatetime());
		return stockVo;
	}

	public CompanyStockStructure getStockBy(StockVo stockVo) {
		String userName = loginservice.getCurrentUserName();
		Date date = new Date();
		CompanyStockStructure stock = stockVo.getStock();
		if (stock == null) {
			stock = new CompanyStockStructure();
			stock.setCreatedBy(userName);
			stock.setCreatedDatetime(date);
		}
		stock.setUpdatedBy(userName);
		stock.setUpdatedDatetime(date);

		stock.setCompanymstr(stockVo.getCompanymstr());
		stock.setCurrency(stockVo.getCurrency());
		stock.setDefunctInd(stockVo.getDefunctInd());
		stock.setRatio(Double.valueOf(stockVo.getRatio()));
		stock.setRegisteredCapital(new BigDecimal(stockVo.getRegisteredCapital()));
		stock.setShareholder(stockVo.getShareholder());
		stock.setStatisticsDatetime(stockVo.getStatisticsDatetime());
		stock.setType(stockVo.getType());
		return stock;

	}

	public Companymstr findCompanymstrByCompanymstrId(Long companymstrId) {
		return em.find(Companymstr.class, companymstrId);
	}
	
	public CompanyStockStructure findStockBuStockId(Long stockId){
		return em.find(CompanyStockStructure.class, stockId);
	}

	public void saveStock(StockVo stockVo){
		CompanyStockStructure stock = getStockBy(stockVo);
		if(stock.getId() == null){
			em.persist(stock);
		} else {
			em.merge(stock);
	...
	public void saveStocks(List<StockVo> stockVos){
		for (StockVo stockVo : stockVos) {
			saveStock(stockVo);
		}
	}
	
	public List<StockVo> findStockVosByLatestDate(Long companyId){
		StringBuilder jpql = new StringBuilder();
		jpql.append(" select c from CompanyStockStructure c");
		jpql.append(" where c.companymstr.id = ").append(companyId);
		jpql.append(" and c.statisticsDatetime = (select max(s.statisticsDatetime) from CompanyStockStructure s)");
		List<CompanyStockStructure> stocks = em.createQuery(jpql.toString()).getResultList();
		List<StockVo> stockVos = new ArrayList<StockVo>();
		for (CompanyStockStructure stock : stocks) {
			stockVos.add(getStockVoBy(stock));
	...
	public boolean isExistedStock(StockVo stockVo) {
		if ("Y".equals(stockVo.getDefunctInd())) {
			return false;
		}
		StringBuilder jpql = new StringBuilder();
		jpql.append(" select s from CompanyStockStructure s");
		jpql.append(" where s.defunctInd = 'N'");
		jpql.append(" and s.companymstr.id = ").append(stockVo.getCompanymstr().getId());
		jpql.append(" and s.shareholder = '" + stockVo.getShareholder() + "'");
		jpql.append(" and s.statisticsDatetime = '" + DateUtils.format(stockVo.getStatisticsDatetime(), DateUtils.formatStr_yyyyMMddHHmmss) + "'");
		CompanyStockStructure stock = stockVo.getStock();
		if(stock != null){
			jpql.append(" and s.id <> ").append(stock.getId());
		}
		List<CompanyStockStructure> stocks = em.createQuery(jpql.toString()).getResultList();
	...
	public List searchStockNew(long companyid, Map<String, Object> searchMap, String date) {
		...
		if (date != null) {
			...
			if ((startdate != null && !startdate.equals("")) && (endDate != null && !endDate.equals(""))) {
				sb.append(" and c.statisticsDatetime between '").append(sm.format(startdate)).append("' and '").append(sm.format(endDate))
						.append("'");
		...
		String sql = "select sum (b.REGISTERED_CAPITAL),b.STATISTICS_DATETIME from COMPANY_STOCK_STRUCTURE as b where COMPANYMSTR_ID= " + companyid
				+ " group by b.STATISTICS_DATETIME ";
	...
	public Date getLastStock(long companyid) {
		String sql = " select c.statisticsDatetime from CompanyStockStructure  c where c.companymstr.id = " + companyid
				+ " order by c.statisticsDatetime desc";
	...
	public void deleteStock(long companyid, Date date) {
		this.em.createNativeQuery(
				"delete from Company_Stock_Structure where COMPANYMSTR_ID =" + companyid + " and Company_Stock_Structure.STATISTICS_DATETIME ='"
						+ new SimpleDateFormat("yyyy-MM-dd").format(date) + " 00:00:00.0'").executeUpdate();
	}

	public String getCompanyStockStructureByDate(Date date, long companyid) {
		String sql = "select sum(ss.registeredCapital),ss.currency from CompanyStockStructure ss where ss.statisticsDatetime = '"
				+ new SimpleDateFormat("yyyy-MM-dd").format(date) + " 00:00:00.0' and ss.companymstr.id = " + companyid
				+ " and ss.defunctInd <> 'Y' and ss.companymstr.defunctInd <> 'Y' group by ss.currency";
